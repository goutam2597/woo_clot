...
class PaymentMethodsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final methods = context.watch<PaymentMethodsController>().items;
  ...
  Future<void> _showMethodSheet(BuildContext context, {PaymentMethodModel? initial}) async {
    final ctrl = context.read<PaymentMethodsController>();
    PaymentType type = initial?.type ?? PaymentType.visa;
    final label = TextEditingController(text: initial?.label ?? (type == PaymentType.paypal ? 'PayPal' : type == PaymentType.visa ? 'Visa' : 'Mastercard'));
        ...
        return Padding(
          ...
          child: StatefulBuilder(builder: (ctx, setState) {
            return Column(
              ...
              children: [
                Text(initial == null ? 'Add Payment Method' : 'Edit Payment Method',
                    style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
                const SizedBox(height: 12),
                DropdownButtonFormField<PaymentType>(
                  initialValue: type,
                  items: const [
                    DropdownMenuItem(value: PaymentType.visa, child: Text('Visa')),
                    DropdownMenuItem(value: PaymentType.mastercard, child: Text('Mastercard')),
                    DropdownMenuItem(value: PaymentType.paypal, child: Text('PayPal')),
                  ],
                  onChanged: (v) => setState(() => type = v ?? type),
                  decoration: _inputDecor('Type'),
                ),
                const SizedBox(height: 8),
                TextField(controller: label, decoration: _inputDecor('Label')),
                const SizedBox(height: 8),
                TextField(
                  controller: details,
                  decoration: _inputDecor(type == PaymentType.paypal ? 'PayPal Email' : 'Card (masked) e.g., **** 4242'),
                ),
                const SizedBox(height: 12),
                SizedBox(
                  height: 48,
                  child: ElevatedButton(
                    ...
                    onPressed: () {
                      if (initial == null) {
                        ...
                        ctrl.update(PaymentMethodModel(
                          id: initial.id,
                          type: type,
                          label: label.text.trim(),
                          details: details.text.trim(),
                    ...
                    child: Text(initial == null ? 'Add Method' : 'Save Changes',
...
class _MethodTile extends StatelessWidget {
  ...
  const _MethodTile({required this.method, required this.onEdit, required this.onDelete});
